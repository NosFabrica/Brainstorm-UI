...
import { ArrowRight, Sparkles, Info, Zap, ChevronRight } from 'lucide-react';
  ...
  return (
    <span 
      className={`font-mono transition-all duration-200 ${
        isScrambling 
          ? "text-indigo-400 text-shadow-[0_0_8px_rgba(129,140,248,0.6)]" 
          : "text-slate-400"
      }`}
    >
      {displayText}
...
export default function Landing() {
  ...
  const { initializeNetwork, generateNewIdentity } = useStore();
  const identityCount = useCountUp(12453, 8000, 1000);
  const relayCount = useCountUp(847, 6000, 1800);
  const connectionCount = useCountUp(2.1, 10000, 1400);
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col relative overflow-hidden">
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          >
            <motion.div 
              className="flex flex-col sm:flex-row items-center lg:items-start gap-4 sm:gap-5 group/brand cursor-pointer relative"
                ...
                <div className="flex flex-col items-center sm:items-start gap-2 mt-4">
                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                    <div className="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-slate-800/50 border border-slate-700/40 rounded-full text-[10px] text-slate-400">
                      ...
                      <span className="tabular-nums font-medium">12,453</span>
                    </div>
                    <div className="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-slate-800/50 border border-slate-700/40 rounded-full text-[10px] text-slate-400">
                      ...
                      <span className="tabular-nums font-medium">847</span>
                    </div>
                    <div className="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-slate-800/50 border border-slate-700/40 rounded-full text-[10px] text-slate-400">
                      ...
                      <span className="tabular-nums font-medium">2.1M</span>
                    </div>
                  </div>
                  {/* Mathematical clarity tooltip */}
          ...
          <motion.div 
            className="relative bg-slate-900/70 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 sm:p-8 shadow-2xl shadow-indigo-500/20 ring-1 ring-indigo-500/10 w-full max-w-md mx-auto lg:mx-0 group/card"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            <div className="flex flex-col gap-3 group/signin">
              <motion.button
                className="relative px-6 py-3 text-base font-medium text-white bg-indigo-600 hover:bg-indigo-100 hover:text-indigo-900 rounded-lg transition-all duration-300 inline-flex items-center justify-center gap-2 w-full overflow-hidden group"
                onClick={() => setLocation('/sign-in?mode=login')}
            ...
            <p className="text-xs text-slate-500 text-center mt-5">
              No password required. <span className="text-slate-300">Just your <a href="https://nostr.how/en/what-is-nostr" target="_blank" rel="noopener noreferrer" className="group-hover/card:text-violet-400 hover:underline transition-colors duration-300 cursor-pointer">Nostr</a> access.</span>
            </p>
          </motion.div>
        </div>
      </motion.div>
