...
export function LoginFailureModal({
  ...
  const isExtensionMode = errorCode === "EXTENSION_FAILED" || errorCode === "PERMISSION_DENIED" || errorCode === "SIGN_CANCELLED";
  const isNoExtension = errorCode === "NO_EXTENSION";

  const headline = isExtensionMode
    ? "Couldn't reach your Nostr extension"
    : isNoExtension
    ? "No Nostr extension detected"
    : "Sign-in failed";

  const subheadline = isExtensionMode
    ? "Unlock your extension and try again, or sign in with your nsec below."
    : isNoExtension
    ? "Install a NIP-07 extension below, or sign in with your nsec."
    : errorMessage;
      ...
      >
        ...
        <div className="relative flex flex-col max-h-[90vh]">
          ...
          <div className="overflow-y-auto flex-1 min-h-0">
            ...
            {errorMessage && (isExtensionMode || isNoExtension) && (
            ...
            {isExtensionMode && (
              <div className="px-4 sm:px-6 pb-3">
                <button
                  ...
                  className="w-full h-10 sm:h-11 rounded-xl bg-white/70 hover:bg-white border border-[#7c86ff]/30 hover:border-[#7c86ff]/50 text-[#333286] font-semibold text-xs sm:text-sm transition-all duration-200 flex items-center justify-center gap-2"
                ...
                <div className="flex flex-wrap gap-2">
                  {EXTENSIONS.map((ext) => (
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/70 border border-[#7c86ff]/20 text-[#333286] text-xs font-semibold hover:bg-white hover:border-[#7c86ff]/40 transition-colors"
