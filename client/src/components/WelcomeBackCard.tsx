...
function Avatar({ j, size = "h-9 w-9" }: { j: NewJoiner; size?: string }) {
  return j.picture ? (
    <img src={j.picture} alt="" width={40} height={40} loading="lazy" className={`${size} rounded-full object-cover ring-1 ring-slate-200 shrink-0`} />
  ) : (
    ...
    return (
      <div className={cardShell} data-testid="card-welcome-back">
        <div className="relative p-5 sm:p-6">
          <button type="button" onClick={() => dismiss(joiners.map((j) => j.pubkey))} className="absolute top-0 right-0 h-9 w-9 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors" aria-label="Dismiss" data-testid="button-welcome-back-dismiss">
            <X className="h-4 w-4" />
          </button>
          ...
          <h3 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight" style={{ fontFamily: "var(--font-display)" }} data-testid="text-welcome-back-title">
          ...
          <ul className="mt-4 space-y-2">
            {joiners.map((j) => (
              <li key={j.pubkey} className="flex items-center gap-3 rounded-xl bg-white/70 border border-slate-100 px-3 py-2" data-testid={`joiner-row-${j.pubkey}`}>
  ...
  if (welcomed.length) {
    ...
    return (
      <div className={cardShell} data-testid="card-welcome-back-success">
        <div className="relative p-5 sm:p-6">
          <button type="button" onClick={() => setWelcomed([])} className="absolute top-0 right-0 h-9 w-9 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors" aria-label="Dismiss" data-testid="button-welcome-success-done">
            <X className="h-4 w-4" />
          </button>
          <div className="flex items-center gap-2.5 mb-2">
          ...
          <div className="mt-4 flex items-center gap-2">
            <div className="flex -space-x-2">
              {welcomed.slice(0, 6).map((j) => (
                <div key={j.pubkey} className="ring-2 ring-white rounded-full">
                  <Avatar j={j} size="h-9 w-9" />
                </div>
              ))}
            </div>
            {n > 6 && <span className="text-sm font-semibold text-slate-500">+{n - 6}</span>}
          </div>
          <button type="button" onClick={() => setWelcomed([])} className="mt-5 inline-flex items-center justify-center rounded-lg border border-slate-200 bg-white px-4 h-9 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-colors">
            Done
          </button>
        </div>
      </div>
...
function InviteCta() {
  ...
  return (
    <div className={cardShell} data-testid="card-invite-cta">
      <div className="relative p-5 sm:p-6">
        <button type="button" onClick={handleDismiss} className="absolute top-0 right-0 h-9 w-9 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors" aria-label="Dismiss" data-testid="button-invite-cta-dismiss">
          <X className="h-4 w-4" />
        </button>
        <div className="flex items-center gap-2.5 mb-2">
