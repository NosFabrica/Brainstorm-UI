...
import { Headphones, Pause, Play, Video } from "lucide-react";
...
import { toggleTrack, useTrackPlayer } from "@/lib/audioPlayer";
...
export type LatestMediaItem =
  ...
  | { kind: "podcast"; id: string; title: string; poster: string | null; audio: string; at: number; href: string; pageUrl: string };
  ...
  useEffect(() => {
    ...
    Promise.all(
      podcastLinks.map(async ({ event, url }) => {
        ...
        return item ? ({ kind: "podcast" as const, id: event.id, title: item.title, poster: item.image, audio: item.audio, at: event.created_at, href: eventPath(event), pageUrl: item.url }) : null;
      }),
    ).then((rows) => {
...
function PodcastRow({ item }: { item: Extract<LatestMediaItem, { kind: "podcast" }> }) {
  const id = `fountain:${item.id}`;
  ...
  return (
    <MediaRow
      ...
      onPlay={() => toggleTrack(id, item.audio)}
