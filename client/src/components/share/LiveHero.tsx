...
export function LiveHero({ event }: { event: MinimalEvent }) {
  ...
  return (
    <div data-testid="live-hero">
          ...
          <div className="relative flex flex-col items-center gap-2 px-6 text-center text-white">
            {isUpcoming ? (
              ...
              <>
                ...
                {watchUrl && (
                  <a href={watchUrl} target="_blank" rel="noopener noreferrer" className="mt-1 inline-flex items-center gap-1.5 rounded-xl bg-white px-4 py-2 text-sm font-semibold text-[#3730a3] transition-colors hover:bg-slate-100" data-testid="live-watch-external">
      ...
      {watchUrl && canEmbed && (
        <a href={watchUrl} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-[#3730a3] hover:underline">
