...
// a link: it sits inside cards that are links themselves. It earns its place
// only where kinds mix on one surface: a tab where every card is a listing
// passes `mixed={false}` and the pill stays away, unless the reader turned on
// labels everywhere (lib/kindLabelsPref).
//
...
export function KindPill({ event, label, mixed = true, tone = "slate", size = "sm", className, ...rest }: { event?: KindEvent; label?: string; /** Whether this surface mixes kinds. Where it holds one, the pill stays away — unless the reader asked for labels everywhere. */ mixed?: boolean } & Omit<ChipProps, "children">) {
  if (!mixed && !kindLabelsEverywhere()) return null;
