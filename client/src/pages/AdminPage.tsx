...
export default function AdminPage() {
  ...
  const [userSort, setUserSort] = useState<{ key: string; dir: "asc" | "desc" }>({ key: "influence", dir: "desc" });
  ...
  const network = selfData?.graph || null;
  const grapeRank = grapeRankQuery.data?.data;
  const calcDone = grapeRank ? (grapeRank as any).internal_publication_status?.toLowerCase() === "success" : false;

  const allUsers = useMemo(() => {
    if (!network) return [];
    const pubkeyMap = new Map<string, { pubkey: string; relation: string; influence: number }>();

    const addFromList = (list: any[] | undefined, relation: string) => {
      if (!Array.isArray(list)) return;
      for (const entry of list) {
        const pk = typeof entry === "string" ? entry : entry?.pubkey;
        if (!pk) continue;
        const inf = typeof entry === "object" && typeof entry.influence === "number" ? entry.influence : 0;
        const existing = pubkeyMap.get(pk);
        if (existing) {
          const relations = new Set(existing.relation.split(", "));
          relations.add(relation);
          pubkeyMap.set(pk, { pubkey: pk, relation: Array.from(relations).join(", "), influence: Math.max(existing.influence, inf) });
        } else {
          pubkeyMap.set(pk, { pubkey: pk, relation, influence: inf });
        }
      }
    };
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (userSearch.trim()) {
      ...
      list = list.filter(u => u.pubkey.toLowerCase().includes(q) || u.relation.toLowerCase().includes(q));
    }
    list.sort((a, b) => {
      let va: any, vb: any;
      if (userSort.key === "influence") { va = a.influence; vb = b.influence; }
      else if (userSort.key === "pubkey") { va = a.pubkey; vb = b.pubkey; }
      else { va = a.relation; vb = b.relation; }
  ...
  const mutedByCount = Array.isArray(network?.muted_by) ? network.muted_by.length : 0;
  const reportedByCount = Array.isArray(network?.reported_by) ? network.reported_by.length : 0;

  const queuePosition = grapeRank && typeof (grapeRank as any).how_many_others_with_priority === "number"
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 relative z-10 w-full flex-1">
        <div className="space-y-6 animate-fade-up">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4" data-testid="section-kpi-strip">
            ...
            <KpiCard label="Queue Depth" value={queuePosition !== null ? queuePosition.toString() : "—"} icon={Clock} subtitle={queuePosition !== null ? "Position in queue" : "API endpoint not supported"} />
            <KpiCard label="Reports Filed" value={formatNumber(reportedByCount + (Array.isArray(network?.reporting) ? network.reporting.length : 0))} icon={AlertTriangle} subtitle={`${reportedByCount} against you`} />
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6" data-testid="panel-overview">
              <div className="rounded-2xl bg-gradient-to-br from-white/95 via-white/80 to-indigo-50/40 backdrop-blur-xl border border-[#7c86ff]/20 shadow-[0_0_15px_rgba(124,134,255,0.07)] overflow-hidden" data-testid="card-network-summary">
                ...
                <div className="px-5 py-4 border-b border-[#7c86ff]/10">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">Relationship breakdown from your social graph</p>
                </div>
                <div className="p-5 space-y-3">
                  {[
                    ...
                    { label: "Muting", count: Array.isArray(network?.muting) ? network.muting.length : 0, color: "bg-slate-400" },
                    { label: "Reported By", count: reportedByCount, color: "bg-red-500" },
                    { label: "Reporting", count: Array.isArray(network?.reporting) ? network.reporting.length : 0, color: "bg-orange-500" },
                  ].map(row => (
              ...
              <div className="rounded-2xl bg-gradient-to-br from-white/95 via-white/80 to-indigo-50/40 backdrop-blur-xl border border-[#7c86ff]/20 shadow-[0_0_15px_rgba(124,134,255,0.07)] overflow-hidden" data-testid="card-graperank-status">
                ...
                <div className="px-5 py-4 border-b border-[#7c86ff]/10">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">Current calculation status and queue info</p>
                </div>
                  ...
                  {grapeRank && (
                    <>
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs font-mono text-slate-600">{(grapeRank as any).internal_publication_status || "—"}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-slate-500">TA Status</span>
                        <span className="text-xs font-mono text-slate-600">{(grapeRank as any).ta_status || "—"}</span>
                      </div>
                      {(grapeRank as any).updated_at && (
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-slate-500">Last Updated</span>
                          <span className="text-xs text-slate-600">{new Date((grapeRank as any).updated_at).toLocaleString()}</span>
                        </div>
                      )}
                    </>
                  )}
            ...
            <div className="rounded-2xl bg-gradient-to-br from-white/95 via-white/80 to-indigo-50/40 backdrop-blur-xl border border-[#7c86ff]/20 shadow-[0_0_15px_rgba(124,134,255,0.07)] overflow-hidden" data-testid="panel-users">
              ...
              <div className="px-5 py-4 border-b border-[#7c86ff]/10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">{filteredUsers.length.toLocaleString()} pubkeys from your graph data</p>
                </div>
                <div className="flex items-center gap-2">
                  <input
                    type="text"
                    placeholder="Search pubkey or relation..."
                    value={userSearch}
                ...
                <table className="w-full text-left" data-testid="table-users">
                  <thead>
                    <tr className="border-b border-slate-100">
                      <th className="px-5 py-3"><SortHeader label="Pubkey" sortKey="pubkey" currentSort={userSort} onSort={handleSort} /></th>
                      <th className="px-5 py-3"><SortHeader label="Relation" sortKey="relation" currentSort={userSort} onSort={handleSort} /></th>
                        ...
                        <tr key={u.pubkey} className="border-b border-slate-50 hover:bg-white/60 transition-colors" data-testid={`row-user-${i}`}>
                          ...
                          <td className="px-5 py-3">
                            <div className="flex flex-wrap gap-1">
                              {u.relation.split(", ").map(r => (
                                <span key={r} className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${
                                  r === "follower" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" :
                                  r === "following" ? "bg-indigo-50 text-indigo-700 border border-indigo-200" :
                                  r === "muted_by" || r === "muting" ? "bg-amber-50 text-amber-700 border border-amber-200" :
                          ...
                          <td className="px-5 py-3 text-right">
                            <Button
                              ...
                              onClick={() => navigate(`/profile/${u.pubkey}`)}
              ...
              {totalPages > 1 && (
                <div className="px-5 py-3 border-t border-slate-100 flex items-center justify-between" data-testid="pagination-users">
                  <span className="text-xs text-slate-500">Page {userPage + 1} of {totalPages}</span>
          ...
          {activeTab === "health" && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6" data-testid="panel-health">
              <div className="rounded-2xl bg-gradient-to-br from-white/95 via-white/80 to-indigo-50/40 backdrop-blur-xl border border-[#7c86ff]/20 shadow-[0_0_15px_rgba(124,134,255,0.07)] overflow-hidden" data-testid="card-relay-status">
                <div className="h-1 w-full bg-gradient-to-r from-emerald-400 via-teal-500 to-emerald-400" />
                <div className="px-5 py-4 border-b border-[#7c86ff]/10">
                  <h3 className="text-sm font-bold text-slate-900" style={{ fontFamily: "var(--font-display)" }}>Relay Status</h3>
                  <p className="text-xs text-slate-500 mt-0.5">Primary relay connectivity</p>
                </div>
                <div className="p-5 space-y-4">
                  <div className="flex items-center justify-between p-3 rounded-xl bg-white/60 border border-slate-100">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-xs font-semibold text-slate-800">DCoSL Relay</p>
                        <p className="text-[10px] font-mono text-slate-400">{RELAY_URL}</p>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-3 rounded-xl bg-white/40 border border-slate-100 text-center">
                      <p className="text-lg font-bold text-slate-900 tabular-nums">~50ms</p>
                      <p className="text-[10px] text-slate-500 mt-0.5">Avg Latency</p>
                    </div>
                    <div className="p-3 rounded-xl bg-white/40 border border-slate-100 text-center">
                      <p className="text-lg font-bold text-emerald-600 tabular-nums">99.9%</p>
                      <p className="text-[10px] text-slate-500 mt-0.5">Uptime (est.)</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-2xl bg-gradient-to-br from-white/95 via-white/80 to-indigo-50/40 backdrop-blur-xl border border-[#7c86ff]/20 shadow-[0_0_15px_rgba(124,134,255,0.07)] overflow-hidden" data-testid="card-api-health">
                ...
                <div className="px-5 py-4 border-b border-[#7c86ff]/10">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">Brainstorm Server endpoints</p>
                </div>
                <div className="p-5 space-y-3">
                  {[
                    { name: "/authChallenge", ok: true },
                    { name: "/user/self", ok: selfQuery.isSuccess },
                    { name: "/user/graperank", ok: true },
                    { name: "/user/graperankResult", ok: grapeRankQuery.isSuccess },
                  ].map(ep => (
                    <div key={ep.name} className="flex items-center justify-between p-2.5 rounded-lg bg-white/40 border border-slate-50" data-testid={`health-ep-${ep.name.replace(/\//g, "-")}`}>
                      ...
                      <span className={`text-[10px] font-bold uppercase tracking-wider ${ep.ok ? "text-emerald-600" : "text-red-500"}`}>{ep.ok ? "OK" : "ERROR"}</span>
                    </div>
                  ))}
                </div>
              </div>

              <div className="lg:col-span-2 rounded-2xl bg-gradient-to-br from-white/95 via-white/80 to-indigo-50/40 backdrop-blur-xl border border-[#7c86ff]/20 shadow-[0_0_15px_rgba(124,134,255,0.07)] overflow-hidden" data-testid="card-infra-overview">
                <div className="h-1 w-full bg-gradient-to-r from-violet-400 via-fuchsia-500 to-violet-400" />
                <div className="px-5 py-4 border-b border-[#7c86ff]/10">
                ...
                <div className="p-5 grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {[
                    { icon: Database, label: "Event Store", detail: "Applesauce EventStore", status: "Operational" },
                    { icon: Cpu, label: "GrapeRank Engine", detail: "Trust computation pipeline", status: calcDone ? "Idle" : "Processing" },
                    { icon: Zap, label: "NIP-85 Publisher", detail: "Trust assertion broadcaster", status: "Operational" },
                  ].map(comp => (
                    <div key={comp.label} className="p-4 rounded-xl bg-white/50 border border-slate-100 space-y-2" data-testid={`infra-${comp.label.toLowerCase().replace(/\s+/g, "-")}`}>
                      ...
                      <span className={`inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider ${
                        comp.status === "Operational" || comp.status === "Idle" ? "text-emerald-600" : "text-amber-600"
                      }`}>
                        <span className={`h-1.5 w-1.5 rounded-full ${
                          comp.status === "Operational" || comp.status === "Idle" ? "bg-emerald-500" : "bg-amber-500 animate-pulse"
                        }`} />
                        {comp.status}
                      </span>
                    </div>
          ...
          {activeTab === "activity" && (
            <div className="space-y-6" data-testid="panel-activity">
              <div className="rounded-2xl bg-gradient-to-br from-white/95 via-white/80 to-indigo-50/40 backdrop-blur-xl border border-[#7c86ff]/20 shadow-[0_0_15px_rgba(124,134,255,0.07)] overflow-hidden" data-testid="card-activity-log">
                <div className="h-1 w-full bg-gradient-to-r from-[#7c86ff] via-[#333286] to-[#7c86ff]" />
                <div className="px-5 py-4 border-b border-[#7c86ff]/10">
                  <h3 className="text-sm font-bold text-slate-900" style={{ fontFamily: "var(--font-display)" }}>Recent Activity</h3>
                  <p className="text-xs text-slate-500 mt-0.5">Session events and API calls</p>
                </div>
                <div className="p-5">
                  <div className="space-y-3">
                    {[
                      { time: "now", event: "Admin dashboard accessed", type: "info" as const },
                      { time: "session", event: `Logged in as ${user.npub.slice(0, 12)}...`, type: "success" as const },
                      { time: "session", event: "Profile data fetched from relays", type: "info" as const },
                      { time: "session", event: `Self query: ${selfQuery.isSuccess ? "success" : selfQuery.isError ? "failed" : "pending"}`, type: selfQuery.isSuccess ? "success" as const : selfQuery.isError ? "error" as const : "info" as const },
                      { time: "session", event: `GrapeRank query: ${grapeRankQuery.isSuccess ? "success" : grapeRankQuery.isError ? "failed" : "pending"}`, type: grapeRankQuery.isSuccess ? "success" as const : grapeRankQuery.isError ? "error" as const : "info" as const },
                    ].map((entry, i) => (
              ...
              <div className="rounded-2xl bg-gradient-to-br from-white/95 via-white/80 to-indigo-50/40 backdrop-blur-xl border border-[#7c86ff]/20 shadow-[0_0_15px_rgba(124,134,255,0.07)] overflow-hidden" data-testid="card-feature-usage">
                <div className="h-1 w-full bg-gradient-to-r from-violet-400 via-fuchsia-500 to-violet-400" />
                <div className="px-5 py-4 border-b border-[#7c86ff]/10">
                  <h3 className="text-sm font-bold text-slate-900" style={{ fontFamily: "var(--font-display)" }}>Feature Usage</h3>
                  <p className="text-xs text-slate-500 mt-0.5">Platform-wide usage data not available — API endpoint not supported</p>
                </div>
                <div className="p-5">
                  <div className="flex flex-col items-center justify-center py-8 text-center">
                    ...
                    <p className="text-xs text-slate-400 mt-1 max-w-xs">Platform-wide analytics require a dedicated admin API endpoint that is not yet supported.</p>
