...
## 15. What a person publishes — six `limit:1` probes in one REQ (2026-09-24)

The search rows wear chips for what a person publishes (Shop, Recipes, Live…). One
REQ per person carries six filters `{ kinds, authors:[pk], limit:1, search:"include:spam" }`
— one per category — and EOSE answers in ~100ms (Staci → shop; Zap Cooking →
